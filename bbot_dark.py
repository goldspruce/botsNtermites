...
def get_distance_pixels(x, y):
    """A tiny shortcut function using Pythagorean theorem to find how far an (X,Y) point is from the top-left marker."""
...
class BBot:
    ...
    def decide_and_move(self, current_steps):
        ...
        # Cut the motor entirely if its randomized timer has expired
...
def ui_show_trial1_results(screen, duration_secs, duration_steps, mean_rock_displacement, sd_rock_displacement, mean_bbot_displacement, sd_bbot_displacement, output_folder, mean_stop_time, sd_stop_time, mean_stop_steps, sd_stop_steps, target_mean, target_sd, total_trials):
    ...
    while waiting:
        ...
        draw_text(screen, f"Mean Rock Disp:      {sign_rock}{mean_rock_displacement:.2f} px", 18, 330, 155)
        draw_text(screen, f"Rock Disp SD:        {sd_rock_displacement:.2f} px", 18, 330, 180, (200, 200, 200))
        
        sign_bbot = "" if mean_bbot_displacement < 0 else "+"
        draw_text(screen, f"Mean BBot Disp:      {sign_bbot}{mean_bbot_displacement:.2f} px", 18, 330, 275)
        draw_text(screen, f"BBot Disp SD:        {sd_bbot_displacement:.2f} px", 18, 330, 300, (200, 200, 200))
