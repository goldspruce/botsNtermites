...
# -- SIMULATION MODES --
BLOCK_SPAWN_MODE = "GAUSSIAN"  # Still biased, but looser
...
# Max distance for Peripherality calculation (Center to Corner)
...
def get_block_spawn_pos(mode):
    ...
    while True:
        if mode == "UNIFORM":
            ...
            # -- MODIFICATION HERE --
            # Increased sigma from 40 to 90.
            # This creates a wider "hill" of blocks rather than a sharp "spike".
            sigma = 90 
            x = random.gauss(center_x, sigma)
            y = random.gauss(center_y, sigma)

        # Safety Check (Keep inside walls)
        if (x > safe_min and x < safe_max_x and 
            y > safe_min and y < safe_max_y):
            return x, y

def get_robot_spawn_pos():
    """
    Determines where ROBOTS appear.
    ALWAYS UNIFORM (Randomly scattered across the map).
    """
    safe_min = WALL_THICKNESS + 20
    safe_max_x = WIDTH - WALL_THICKNESS - 20
    safe_max_y = HEIGHT - WALL_THICKNESS - 20
...
class Robot:
    def __init__(self):
        # ROBOTS use the Unbiased (Uniform) spawner
...
class Block:
    def __init__(self):
        # BLOCKS use the Biased (Gaussian) spawner
...
def show_results_screen(screen, blocks, duration):
    ...
    while waiting:
        ...
        # Display Spawn Mode
        draw_text(screen, f"Blocks: {BLOCK_SPAWN_MODE} (Loose) | Robots: UNIFORM", 20, 30, 70, (100, 200, 255))
...
# =============================================================================
def main():
    print(f"Initializing Simulation (Blocks: {BLOCK_SPAWN_MODE}, Robots: UNIFORM)...")
    pygame.init()
    pygame.font.init() 
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption(f"Type I Swarm: Looser Gaussian Spawning")
    ...
    while running:
        ...
        if simulation_active:
            ...
            draw_text(screen, f"Block Spawn: {BLOCK_SPAWN_MODE} (Looser)", 16, 20, 20, (150, 150, 150))
