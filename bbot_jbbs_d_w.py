...
VENT_STRENGTH_WALL = 0.08          # Outer edge: Strong pinning into the wall
VENT_STRENGTH_SUB_WALL = 0.0      # One step in: Weak pinning into the wall
VENT_STRENGTH_INTERNAL = 0.1      # Deep center: Random turbulent jiggle

VENT_REACH = 60                   # Range of influence
THERMAL_KICK = 1.6                # Base noise temperature
BLOCK_FRICTION = 0.90             
RESTITUTION = 0.5
# trial 1
...
# GUESS
VENT_STRENGTH_WALL     = 0.14500  # Pinned harder against the far walls
VENT_STRENGTH_SUB_WALL = 0.00000  # Keep the buffer
VENT_STRENGTH_INTERNAL = 0.13500  # Higher internal turbulence pushing them out
THERMAL_KICK           = 1.60000  # Keep thermal baseline standard

# zeros
VENT_STRENGTH_WALL     = 0  # Pinned harder against the far walls
VENT_STRENGTH_SUB_WALL = 0  # Keep the buffer
VENT_STRENGTH_INTERNAL = 0  # Higher internal turbulence pushing them out
THERMAL_KICK           = 1  # Keep thermal baseline standard
# =============================================================================
...
def export_sampled_results(all_trials_data, output_folder):
    ...
    with open(csv_path, mode='w', newline='') as file:
        ...
        for r in all_trials_data:
            row = [r['trial']] + [r['time_series'][step] for step in sample_steps]
            writer.writerow(row)
        ...
        for step in sample_steps:
            step_mean = statistics.mean([r['time_series'][step] for r in all_trials_data])
...
def run_simulation(trial_num, is_visual, output_folder, screen=None):
    ...
    # Spawn rocks one by one, passing the growing list to the Block class
...
def main():
    ...
    plot_histogram(pooled_final_displacements, 'Final Rock Displacements (Step 7000)', 'Net Displacement (pixels)', 'Frequency', "04_final_displacement_hist.png", 'forestgreen', output_folder)
