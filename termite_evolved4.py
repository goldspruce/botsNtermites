...
best_gen_score = 0.0
best_gen_info = "N/A"
best_species_idx_log = 0 # Track index for logging

saved_evol_snapshots = [] 
saved_sim_snapshots = []
saved_sim_metrics = [] 

# Clear previous log file if it exists to start fresh
if os.path.exists("evolution_log.txt"):
    os.remove("evolution_log.txt")
...
def calculate_order_fast(grid, total_bricks):
    """
    Calculates the 'Clumpiness' metric.
    Math: Sum of all orthogonal neighbors / (Total Bricks * 4).
    Returns 0.0 (Disorder) to ~1.0 (Perfect solid block).
    """
...
def run_simulation_headless(genome):
    """
    Runs the simulation without graphics for training.
    Used during the Genetic Algorithm phase to assess fitness.
    """
...
def mutate(genome):
    """
    Standard mutation: pick one gene, shift it by +/- 0.2.
    """
...
# Placeholders for Simulation Phase
ax_sim_grid = None
ax_sim_graph = None
img = None
line_sim = None
...
def start_simulation_phase():
    ...
    add_black_border(ax_sim_grid)
    
    genes_pick = [f"{x:.2f}" for x in best_genome_overall[0:5]]
    genes_drop = [f"{x:.2f}" for x in best_genome_overall[5:10]]
    
    # --- NEW: Added Generation Origin to Title ---
    caption_text = (
        f"RESULT FROM GENERATION {GENERATIONS}\n" # Explicitly stating origin
        f"Species ID: {best_info_overall}\n" 
    ...
    ax_sim_graph.set_title(f"Real-Time Construction Order (Gen {GENERATIONS})")
...
def update(frame):
    ...
    if current_state == STATE_EVOLVING:
        if current_gen < GENERATIONS:
            ...
            # Extract Best of Generation
            best_gen_score, best_gen_genome, best_species_idx = scores[0]
            best_species_idx_log = best_species_idx # Save for logging
            best_gen_info = f"Gen {current_gen + 1}, Species #{best_species_idx}"
            
            # --- NEW: WRITE TO LOG FILE -------------------------------
            # This appends the best genome of the current generation to a text file.
            # Format: Generation | Score | Genome List
            with open("evolution_log.txt", "a") as f:
                f.write(f"Generation {current_gen + 1} | Score: {best_gen_score:.5f} | Species #{best_species_idx}\n")
                f.write(f"Genome: {best_gen_genome}\n")
                f.write("-" * 50 + "\n")
            # ----------------------------------------------------------
            ...
            # Snapshot Logic for Evolution Phase
            check_gen = current_gen + 1
            if check_gen in [5, 10, 15, 20]:
                filename = f"evol_snapshot_gen_{check_gen}.png"
                # The text_display already contains the generation info, so we just save.
    ...
    # -----------------------------
        ...
        for _ in range(STEPS_PER_FRAME):
            ...
            if sim_step in [2500, 5000, 10000]:
                ...
                # We update the title immediately before saving to ensure the Gen Number is there
                original_title = ax_sim_grid.get_title(loc='left')
                ax_sim_grid.set_title(original_title + f"\n[SNAPSHOT @ Step {sim_step}]", fontsize=10)
                
                extent = ax_sim_grid.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
                plt.savefig(filename, bbox_inches=extent.expanded(1.05, 1.05)) 
                
                # Restore title
                ax_sim_grid.set_title(original_title, fontsize=10, loc='left')
    ...
    # -----------------------------
        ...
        if not slide_drawn:
            ...
            # SLIDE 4: Final Construction Sequence
                ...
                # --- NEW: Added Header with Generation Number ---
                fig.suptitle(f"Final Construction Sequence (Best Agent from Gen {GENERATIONS})", fontsize=16, fontweight='bold')
...
if __name__ == "__main__":
    ...
    print(f"LOG:  Results will be saved to evolution_log.txt")
    print("------------------------------------------------")
